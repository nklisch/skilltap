---
id: feature-cleanup-current-state
kind: feature
stage: review
tags: [cleanup, content, refactor]
created: 2026-05-19
---

> Original design migrated from `docs/designs/cleanup-current-state.md`.
> Most code units (plugin-v2 rename, e2e-v2 rename, InstalledJson schema drop)
> have landed in main. Doc/website/ROADMAP units have also largely landed
> based on the current state of those files. Outstanding work: confirm Unit 24
> (delete `docs/designs/completed/`) and Unit 25 (`llms-full.txt` regen) are
> complete, then close out.
