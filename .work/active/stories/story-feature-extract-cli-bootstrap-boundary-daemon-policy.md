...
**Notes**: Substrate review, standard weight. `entrypoint` retains only the
thin daemon-cycle composition wrapper; policy and destination lookup are
private to `bootstrap_commands`. Daemon policy fixtures, lifecycle tests, and
full CLI verification remain green with no output or ordering changes.
