...
async function processSkillFile(skillMdPath: string): Promise<ScannedSkill> {
  ...
  try {
    ...
    return { name: dirName, description: "", path: skillDir, valid: false, warnings: ["Could not read SKILL.md"] }
  }

  const data = parseSkillFrontmatter(content)
  if (!data) {
    warnings.push("No YAML frontmatter found — using directory name as skill name")
    return { name: dirName, description: "", path: skillDir, valid: false, warnings }
  }
  ...
  if (fm.name !== dirName) {
    warnings.push(`Skill name "${fm.name}" does not match directory name "${dirName}"`)
  }

  return { name: fm.name, description: fm.description, path: skillDir, valid: true, warnings }
}
...
function isAgentsSkillsPath(skillPath: string): boolean {
  return skillPath.includes("/.agents/skills/") || skillPath.includes("\\.agents\\skills\\")
}
...
export async function scan(dir: string): Promise<ScannedSkill[]> {
  ...
  for await (const rel of agentsGlob.scan({ cwd: dir, onlyFiles: true, dot: true })) {
