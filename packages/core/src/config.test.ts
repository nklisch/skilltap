...
import { ensureDirs, loadConfig, saveConfig, loadInstalled, saveInstalled } from "./config"
...
describe("loadConfig", () => {
  test("creates default config file when missing", async () => {
    ...
    expect(result.ok).toBe(true)
    expect(await fileExists(join(tmpDir, "skilltap", "config.toml"))).toBe(true)
  })
...
describe("loadInstalled", () => {
  ...
  test("returns error for invalid schema", async () => {
    ...
    const result = await loadInstalled()
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.error.message).toContain("Invalid installed.json")
  })
})
...
describe("saveInstalled", () => {
  test("writes installed.json to disk", async () => {
    ...
    expect(result.ok).toBe(true)
    expect(await fileExists(join(tmpDir, "skilltap", "installed.json"))).toBe(true)
  })
