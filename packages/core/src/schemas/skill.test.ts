...
describe("SkillFrontmatterSchema", () => {
  ...
  test("accepts all optional fields", () => {
    ...
    if (result.success) {
      ...
      expect(result.data.metadata).toEqual({ author: "nathan", version: "1.0" })
    }
  })

  describe("name validation", () => {
    test("accepts lowercase alphanumeric", () => {
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: "abc123" }).success).toBe(true)
    })

    test("accepts hyphen-separated segments", () => {
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: "my-skill" }).success).toBe(true)
      expect(
    ...
    test("rejects uppercase letters", () => {
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: "MySkill" }).success).toBe(false)
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: "MY-SKILL" }).success).toBe(false)
    })

    test("rejects leading hyphen", () => {
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: "-skill" }).success).toBe(false)
    })
    ...
    test("rejects consecutive hyphens", () => {
      expect(
        SkillFrontmatterSchema.safeParse({ ...VALID, name: "skill--name" }).success
      ).toBe(false)
    })

    test("rejects underscores", () => {
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: "my_skill" }).success).toBe(false)
    })
    ...
    test("rejects name longer than 64 chars", () => {
      ...
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: long }).success).toBe(false)
    })

    test("accepts name exactly 64 chars", () => {
      // 64 chars of valid pattern: alternating letter segments
      const name = "a" + "-a".repeat(31) // "a-a-a-..." = 1 + 31*2 = 63 chars, need 64
      // Just use a 64-char lowercase alphanumeric string
      const exact64 = "a".repeat(64)
      expect(SkillFrontmatterSchema.safeParse({ ...VALID, name: exact64 }).success).toBe(true)
    })
  })
  ...
  describe("description validation", () => {
    ...
    test("rejects description longer than 1024 chars", () => {
      ...
      expect(
        SkillFrontmatterSchema.safeParse({ ...VALID, description: long }).success
      ).toBe(false)
    })
    ...
    test("accepts description exactly 1024 chars", () => {
      ...
      expect(
        SkillFrontmatterSchema.safeParse({ ...VALID, description: exact }).success
      ).toBe(true)
    })
  })
  ...
  describe("compatibility validation", () => {
    test("rejects compatibility longer than 500 chars", () => {
      ...
      expect(
        SkillFrontmatterSchema.safeParse({ ...VALID, compatibility: long }).success
      ).toBe(false)
    })
    ...
    test("accepts compatibility exactly 500 chars", () => {
      ...
      expect(
        SkillFrontmatterSchema.safeParse({ ...VALID, compatibility: exact }).success
      ).toBe(true)
    })
  })
  ...
  test("metadata accepts arbitrary key-value pairs", () => {
    const result = SkillFrontmatterSchema.safeParse({
      ...VALID,
      metadata: { author: "alice", tags: ["git", "ci"], count: 42, nested: { x: true } },
    })
    expect(result.success).toBe(true)
  })
