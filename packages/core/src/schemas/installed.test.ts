...
describe("InstalledSkillSchema", () => {
  ...
  test("accepts all scope values", () => {
    for (const scope of ["global", "project", "linked"] as const) {
      expect(
        InstalledSkillSchema.safeParse({ ...VALID_SKILL, scope }).success,
      ).toBe(true);
    }
  });
  ...
  test("rejects invalid scope", () => {
    expect(
      InstalledSkillSchema.safeParse({ ...VALID_SKILL, scope: "local" })
        .success,
    ).toBe(false);
  });
  ...
  test("accepts multi-skill path", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success)
      expect(result.data.path).toBe(".agents/skills/commit-helper");
  });
});
...
describe("InstalledJsonSchema", () => {
  ...
  test("rejects wrong version", () => {
    expect(
      InstalledJsonSchema.safeParse({ version: 2, skills: [] }).success,
    ).toBe(false);
    expect(
      InstalledJsonSchema.safeParse({ version: 0, skills: [] }).success,
    ).toBe(false);
  });
