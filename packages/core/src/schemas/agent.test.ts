...
describe("AgentResponseSchema", () => {
  ...
  test("accepts score 0 (minimum)", () => {
    expect(
      AgentResponseSchema.safeParse({ score: 0, reason: "Safe" }).success,
    ).toBe(true);
  });

  test("accepts score 10 (maximum)", () => {
    expect(
      AgentResponseSchema.safeParse({ score: 10, reason: "Dangerous" }).success,
    ).toBe(true);
  });

  test("rejects score below 0", () => {
    expect(
      AgentResponseSchema.safeParse({ score: -1, reason: "x" }).success,
    ).toBe(false);
  });

  test("rejects score above 10", () => {
    expect(
      AgentResponseSchema.safeParse({ score: 11, reason: "x" }).success,
    ).toBe(false);
  });

  test("rejects non-integer score", () => {
    expect(
      AgentResponseSchema.safeParse({ score: 5.5, reason: "x" }).success,
    ).toBe(false);
  });
  ...
  test("accepts empty reason string", () => {
    expect(
      AgentResponseSchema.safeParse({ score: 0, reason: "" }).success,
    ).toBe(true);
  });
});
...
describe("ResolvedSourceSchema", () => {
  ...
  test("rejects missing url", () => {
    expect(ResolvedSourceSchema.safeParse({ adapter: "git" }).success).toBe(
      false,
    );
  });
  ...
  test("rejects missing adapter", () => {
    expect(
      ResolvedSourceSchema.safeParse({ url: "https://example.com/repo.git" })
        .success,
    ).toBe(false);
  });
});
