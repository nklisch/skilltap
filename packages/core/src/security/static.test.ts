...
describe("scanStatic — malicious skill fixture", () => {
  test("detects invisible Unicode", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Invisible Unicode")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects HTML comment", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "HTML comment")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Markdown comment", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Markdown comment")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Base64 block", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Base64 block")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Hex encoding", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Hex encoding")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Suspicious URL", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Suspicious URL")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Shell command", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Shell command")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Sensitive path", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Sensitive path")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("detects Tag injection", async () => {
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (!result.ok) return
      expect(result.value.some((w) => w.category === "Tag injection")).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
...
describe("scanStatic — file type checks", () => {
  test("flags .wasm files as unexpected file type", async () => {
    ...
    await Bun.write(join(tmpDir, "module.wasm"), new Uint8Array([0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00]))

    const result = await scanStatic(tmpDir)
    expect(result.ok).toBe(true)
    if (!result.ok) return
    expect(result.value.some((w) => w.category === "Unexpected file type")).toBe(true)
  })
  ...
  test("flags ELF binary by magic bytes", async () => {
    await Bun.write(join(tmpDir, "SKILL.md"), "---\nname: test\ndescription: test\n---\n# Test")
    // ELF magic: \x7fELF
    await Bun.write(join(tmpDir, "binary"), new Uint8Array([0x7f, 0x45, 0x4c, 0x46, 0x02, 0x01]))
