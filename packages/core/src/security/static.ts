...
import { ok, err, ScanError } from "../types"
...
const FLAGGED_EXTENSIONS = new Set([".wasm", ".pyc", ".class", ".zip", ".tar", ".gz"])
...
const BINARY_SIGNATURES: Array<{ sig: Uint8Array; category: string }> = [
  ...
  { sig: new Uint8Array([0x00, 0x61, 0x73, 0x6d]), category: "WebAssembly binary" },
]
...
export async function scanStatic(
  ...
  try {
    ...
    for (const relPath of relPaths) {
      ...
      if (ext === ".js" || ext === ".ts") {
        const singleLongLine = content.split("\n").some((line) => line.length > 500)
        if (singleLongLine) {
          warnings.push({ file: relPath, line: 0, category: "Minified code", raw: "Single line exceeds 500 characters" })
        }
      }
    ...
    return err(new ScanError(`Static scan failed: ${e instanceof Error ? e.message : String(e)}`))
  }
}
