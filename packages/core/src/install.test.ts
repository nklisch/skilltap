...
import { $ } from "bun"
import { installSkill, removeSkill, findProjectRoot } from "./install"
...
describe("installSkill — standalone", () => {
  test("installs to global scope", async () => {
    ...
    try {
      ...
      expect(await lstat(join(skillDir, ".git")).then(() => true).catch(() => false)).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
  ...
  test("creates agent symlinks when also is set", async () => {
    ...
    try {
      ...
      const expectedTarget = join(homeDir, ".agents", "skills", "standalone-skill")
...
describe("installSkill — multi-skill", () => {
  test("installs a single selected skill", async () => {
    ...
    try {
      const result = await installSkill(repo.path, { scope: "global", skillNames: ["skill-a"] })
      expect(result.ok).toBe(true)
      if (!result.ok) return
  ...
  test("installs all skills when skillNames omitted", async () => {
    ...
    try {
      ...
      expect(await lstat(join(homeDir, ".agents", "skills", "skill-b")).then((s) => s.isDirectory())).toBe(true)
    } finally {
      await repo.cleanup()
    }
  })
...
describe("installSkill — project scope", () => {
  test("installs to project root", async () => {
    ...
    try {
      ...
      const result = await installSkill(repo.path, { scope: "project", projectRoot })
      expect(result.ok).toBe(true)

      const skillDir = join(projectRoot, ".agents", "skills", "standalone-skill")
...
describe("removeSkill", () => {
  ...
  test("removes cache when last skill from multi-skill repo is removed", async () => {
    const repo = await createMultiSkillRepo()
    try {
      await installSkill(repo.path, { scope: "global", skillNames: ["skill-a"] })
      const installedResult = await loadInstalled()
      if (!installedResult.ok) return
      const record = installedResult.value.skills[0]!
