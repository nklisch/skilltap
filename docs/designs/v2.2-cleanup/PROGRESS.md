...
| 4d | 4 | 4.10, 4.11 + VitePress theme components | done | 15 files (+671/-568). 11 guide pages aligned (configuration/security/doctor/teams full rewrites; installing-skills + shell-completions substantial; installation/creating-skills/taps/skill-format already aligned). 4 reference pages aligned (cli.md per-command flag tables match citty 1:1; tap-format.md HTTP-registry section deleted). LandingPage.vue + InstallTabs.vue + SecurityScanDemo.vue + TerminalDemo.vue user-visible copy swept. One straggler: `taps.md` line ~112 references `skilltap tap install` (not wired in code). Agent `aeeda8167b3d08aec`, commit `221c2b0` |
| 4e | 4 | 4.12 + final verification + lint sweep | done | `taps.md` `tap install` straggler scrubbed. `llms-full.txt` regenerated (3984 lines). Lint cleaned (`as any` → typed unknown casts in install commands + dropped 5 stale biome-ignore directives + auto-fixes via `biome check --write --unsafe` across 127 files). All 29 verification gates pass. Agent `ac4953f30f2f8590c` (delivered taps.md + llms-full.txt regen but cut off mid-report). Commits `512083a` + `b1e6e29` |
| Final | — | — | done | All 29 gates PASS. Suite 2199 pass / 1 baseline fail (pre-existing `taps.http-removal.test.ts`) / 51 skip. `verify:binary:tests` 588/588. Build clean. Lint clean. v2.2 cutover **complete and ready to tag.** |
...
- **2026-05-09 Wave 4d** — Units 4.10, 4.11 done + VitePress theme component sweep. 15 files (+671/-568). website/guide/: configuration/security/doctor/teams full rewrites; installing-skills (+29 lines for multi-plugin + capture sections); shell-completions; getting-started; what-is-skilltap. website/reference/: cli.md (382→521, per-command flag tables match citty 1:1); config-options.md (only V2 keys + `[[mcps]]`); tap-format.md (-87 lines, HTTP Registry section deleted). LandingPage.vue + InstallTabs.vue + SecurityScanDemo.vue + TerminalDemo.vue swept (replaced `--global` → `--scope`, `skilltap install foo/bar` → `install skill foo/bar`, removed "Agent mode" feature card). One open item: `taps.md` references `skilltap tap install` (not wired in code) — flagged for Wave 4e. Suite still baseline. Agent `aeeda8167b3d08aec`. Commit `221c2b0`.
- **2026-05-09 Wave 4e** — Final close-out. `taps.md` straggler scrubbed (replaced `skilltap tap install` with the actual `find` + `install skill` workflow). `website/public/llms-full.txt` regenerated from the now-aligned source pages (~4000 lines, only one-line removal entries from changelog/migration/Removed-in-v22 sections). Lint sweep cleaned residual issues: `(args as any)._` → `(args as unknown as { _: string[] })._` in install/skill.ts, install/plugin.ts, install/mcp.ts; dropped 5 stale `biome-ignore` directives; `biome check --write --unsafe` auto-fixed 127 files (mostly import organization). All 29 verification gates PASS. Suite 2199 pass / 1 baseline / 51 skip. `verify:binary:tests` 588/588. Build + lint clean. Commits `512083a` (regen + scrub) + `b1e6e29` (lint sweep). Agent `ac4953f30f2f8590c` partial (regen + scrub before report cutoff); orchestrator finished verification + lint manually. **v2.2 complete.**

## Completion summary

The v2.2 V2 cutover shipped across 11 waves over two calendar days (2026-05-08 and 2026-05-09):

- **Phase 1** (Waves 1A/1B/1C): V2 schema, `policy/` module promotion, hard-fail loadConfig, V2 migrate with `state.mcpServers` preservation, manifest `[[mcps]]` schema + lockfile `[[mcps.lock]]`, sync engine reconciles all three state types.
- **Phase 2** (Wave 2): -1213 lines from 7 deleted files (`verify`, `httpAdapter`, `linkSkill`, `searchSkillsRegistry`, `marketplaceSourceToRepo`, `validate.ts`); 6 removed-command hints wired.
- **Phase 3** (Waves 3a/3b/3c/3d): 8 code cleanups + CLI surface alignment (`--scope`, typed `try`, drop `--no-strict`, repeatable `--also`, mcp smart-scope) + multi-plugin syntax + capture flags + lifecycle drift fix + drift.ts `range`/`ref` + TUI fixes (Dashboard tabs / Adopt Enter / Toggle name-step + focusIndex) + doctor exit-code-after-fix + `--json` field completeness + tui.smoke binary routing + 5 new CLI test files.
- **Phase 4** (Waves 4a/4b/4c/4d/4e): Foundation docs, SPEC.md (3576→2070 lines, 158k→73k), UX/changelog/website/index/README, website/guide (11 pages) + website/reference (4 pages), VitePress theme components, `llms-full.txt` regen.

Suite final: **2199 pass / 1 fail (pre-existing `taps.http-removal.test.ts` baseline) / 51 skip**.
`verify:binary:tests`: **588 pass / 0 fail**.
Build: clean.
Lint: clean.

The single failing test (`taps.http-removal.test.ts:89` — the v0.x HTTP-tap stderr warning is no longer emitted in V2) was pre-existing entering this run and out of scope for the cleanup. All other audit findings are closed.

v2.2 is ready to tag.
