...
fn bootstrap_reports_binary_resolution_and_harness_attention_separately() {
    ...
    let output = machine
        .run_with_env(
            ...
            &["bootstrap", "--json"],
            [(
                "SKILLTAP_RELEASE_MANIFEST",
                Path::new("/definitely/missing/manifest.json"),
            )],
        )
        .expect("run unavailable bootstrap");
    assert_code(&output, 2);
    let value = json(&output);
...
fn write_bootstrap_fixture(
    ...
    let root = machine.home().join("bootstrap-fixture");
    fs::create_dir_all(&root).expect("create bootstrap fixture");
    let artifact = root.join("artifact");
    fs::write(&artifact, b"test-binary").expect("write bootstrap artifact");
    let manifest = root.join("manifest.json");
    let manifest_body = format!(
        r#"{{"version":"{version}","assets":[{{"platform":"linux","arch":"x86_64","name":"skilltap","sha256":"6f850b10b66ff0af26a1151306cbc59427ae2572330935d95ab5febdef8763bf","download_url":"https://github.com/nklisch/skilltap/releases/download/v{version}/skilltap"}}]}}"#
    );
    fs::write(&manifest, manifest_body).expect("write bootstrap manifest");
    let destination = root.join("bin/skilltap");
    (manifest, artifact, destination)
}

#[test]
fn bootstrap_installs_verified_fixture_and_keeps_harness_attention_separate() {
    let machine = machine();
    let (manifest, artifact, destination) = write_bootstrap_fixture(&machine, "3.0.1");
    let output = machine
        .run_with_env(
            &binary(),
            &["bootstrap", "--target", "claude", "--json"],
            [
                ("SKILLTAP_RELEASE_MANIFEST", manifest.as_path()),
                ("SKILLTAP_RELEASE_ARTIFACT", artifact.as_path()),
                ("SKILLTAP_INSTALL", destination.as_path()),
            ],
        )
        .expect("run bootstrap fixture");
    assert_code(&output, 2);
    let value = json(&output);
    assert_eq!(value["resources"][0]["status"], "installed");
    assert_eq!(value["resources"][1]["id"], "claude");
    assert!(destination.is_file());
}

#[test]
fn bootstrap_same_major_repeat_is_a_noop_and_major_upgrade_requires_acknowledgment() {
    let machine = machine();
    let (manifest, artifact, destination) = write_bootstrap_fixture(&machine, "3.0.1");
    fs::create_dir_all(destination.parent().unwrap()).unwrap();
    fs::write(&destination, b"test-binary").unwrap();
    let output = machine
        .run_with_env(
            &binary(),
            &["bootstrap", "--target", "codex", "--json"],
            [
                ("SKILLTAP_RELEASE_MANIFEST", manifest.as_path()),
                ("SKILLTAP_RELEASE_ARTIFACT", artifact.as_path()),
                ("SKILLTAP_INSTALL", destination.as_path()),
                ("SKILLTAP_INSTALLED_VERSION", Path::new("3.0.1")),
            ],
        )
        .expect("run no-op bootstrap");
    assert_code(&output, 2);
    assert_eq!(json(&output)["resources"][0]["status"], "no-op");

    let (major_manifest, _, _) = write_bootstrap_fixture(&machine, "4.0.0");
    let output = machine
        .run_with_env(
            &binary(),
            &["bootstrap", "--target", "codex", "--json"],
            [
                ("SKILLTAP_RELEASE_MANIFEST", major_manifest.as_path()),
                ("SKILLTAP_RELEASE_ARTIFACT", artifact.as_path()),
                ("SKILLTAP_INSTALL", destination.as_path()),
                ("SKILLTAP_INSTALLED_VERSION", Path::new("3.0.1")),
            ],
        )
        .expect("run blocked major bootstrap");
    assert_code(&output, 2);
    assert_eq!(
        json(&output)["resources"][0]["status"],
