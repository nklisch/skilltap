...
| `--agent` | boolean | `false` | Force non-interactive agent mode for this invocation. Equivalent to `SKILLTAP_AGENT=1`. Auto-yes; security warnings hard-fail; plain-text output (no spinners). |
...
| `--agent` | boolean | `false` | Force non-interactive agent mode. Equivalent to `SKILLTAP_AGENT=1`. |

### Behavior

When no name is given, shows an interactive multiselect of all installed skills (no separate confirmation step). If a skill is installed at both global and project scopes, the picker shows disambiguated entries with `(global)` / `(project)` labels. When names are supplied, first checks `state.json`; if not found, discovers on disk via `discoverSkills()` — unmanaged skills can be removed too. `--global`/`--project` overrides the stored scope when resolving where to remove from. For each skill, removes the skill directory, any agent-specific symlinks (from the `also` list), and the cache entry if this was the last skill from that repo. Updates `state.json` after each removal.
...
Scans `.agents/skills/` and every agent-specific directory (`.claude/skills/`, `.cursor/skills/`, etc.) at both global and project scope. Skills are classified as **managed** (tracked in `state.json`), **linked** (symlinked via `skilltap link`), or **unmanaged** (manually placed, not in state).
...
Requires exactly one of `--global` or `--project`. Looks up skill in `state.json`, moves the directory, removes old agent symlinks and creates new ones at the target scope. Updates `state.json` records across files when crossing scope boundaries.
...
| `--agent` | boolean | `false` | Force non-interactive agent mode (auto-yes, hard-fail on warnings, plain-text output). Equivalent to `SKILLTAP_AGENT=1`. |
...
8. Update `state.json` with new SHA and `updatedAt`
...
Resolves the path to absolute, validates SKILL.md exists, parses frontmatter for the skill name, and creates a symlink at the install path. Records in `state.json` with `scope: "linked"`.
...
Verifies the skill was linked (not installed via clone), removes the symlink from the install path, removes any agent-specific symlinks, and updates `state.json`.
...
| 4 | installed | Global `state.json` and project `.agents/state.json` (when in a project) parse and pass schema; detail: `"N skills (G global, P project)"`. Falls back to `installed.json` for unmigrated v0.x users. |
